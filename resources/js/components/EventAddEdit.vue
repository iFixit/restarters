  ...
  <div>
    ...
    <div class="eae-layout">
      ...
      <div class="form-group event-description">
        <b-form-group>
          <label for="event_desc">{{ $translate('events.field_event_desc') }}:</label>
          <RichTextEditor
              id="event_desc"
              name="free_text"
              class="moveright"
      ...
      <div class="event-date">
        <label for="event_date">{{ $translate('events.field_event_date') }}:</label>
        <EventDatePicker
            id="event_date"
            class="p-0"
            :date.sync="eventDate"
      ...
      <b-form-group class="event-time mt-3 mt-lg-0">
        <label for="event_time">{{ $translate('events.field_event_time') }}:</label>
      ...
      <b-card v-if="canApprove" no-body class="event-admin">
        <b-card-header>
          ...
          {{ $translate('groups.group_admin_only') }}
        </b-card-header>
        <b-card-body>
      ...
      <div class="event-result text-right">
        <div v-if="failed">
          <p v-if="creating" class="mt-2 text-danger font-weight-bold" v-html="'<div>' + $translate('events.create_failed') + '</div>'"/>
          <p v-else class="mt-2 text-danger font-weight-bold" v-html="'<div>' + $translate('events.edit_failed') + '</div>'"/>
        </div>
      </div>
      ...
      <div class="event-buttons button-group d-flex align-items-center justify-content-between" v-if="creating">
        <div class="d-flex align-items-right justify-content-end text-right flex-grow-1 pr-2 notice">
          <span v-if="autoApprove">
            {{ $translate('events.before_submit_text_autoapproved') }}
          </span>
          <span v-else>
            {{ $translate('events.before_submit_text') }}
          </span>
        </div>
        <div class="col-lg-2 d-flex align-items-center justify-content-end mt-2 mt-lg-0">
          <SpinButton
              icon-name="save"
              :label="$translate('events.create_event')"
              variant="primary"
              @handle="submit"
          />
        </div>
      ...
      <div class="event-buttons button-group d-flex align-items-center justify-content-between" v-else>
        ...
        <div class="d-flex align-items-center justify-content-end mt-2 mt-lg-0">
          ...
          <SpinButton
              icon-name="save"
              :label="$translate('events.save_event')"
              variant="primary"
              @handle="submit"
          />
        </div>
