  ...
  <div>
    ...
    <div class="d-block d-md-none" v-if="search">
      <div class="clickme d-flex justify-content-end pr-3 text-uppercase" v-if="!searchShow" @click="toggleFilters">
        <a href="#">{{ $translate('groups.show_filters') }}</a>&nbsp;<b-img class="plusminusicon" src="/images/add-icon.svg" />
      </div>
      <div class="clickme d-flex justify-content-end pr-3 text-uppercase" v-if="searchShow" @click="toggleFilters">
        <b-img class="plusminusicon" src="/images/minus-icon.svg" /><a href="#">&nbsp;{{ $translate('groups.hide_filters') }}</a>
      </div>
      <GroupsTableFilters
          v-if="searchShow"
    ...
    <b-table :fields="fields" :items="itemsToShow" sort-null-last thead-tr-class="d-none d-md-table-row" :sort-compare="sortCompare">
      ...
      <template slot="cell(next_event)" slot-scope="data">
        <div>
          ...
          <div v-else>
            {{ $translate('groups.upcoming_none_planned') }}
          </div>
        </div>
      </template>
      ...
      <template slot="cell(following)" slot-scope="data">
        <div v-if="approve" class="cell-warning d-flex justify-content-around p-2">
          <a :href="'/group/edit/' + data.item.idgroups">{{ $translate('groups.group_requires_moderation') }}</a>
        </div>
        <b-btn variant="primary" class="text-nowrap mr-2" v-else-if="!data.item.following" :to="'/group/join/' + data.item.idgroups">
          <span class="d-block d-md-none">
            {{ $translate('groups.join_group_button_mobile') }}
          </span>
        ...
        <b-btn variant="primary" class="text-nowrap mr-2" v-else @click="leaveGroup(data.item.idgroups)">
          <span class="d-block d-md-none">
            {{ $translate('groups.leave_group_button_mobile') }}
          </span>
          <span class="d-none d-md-block">
            {{ $translate('groups.leave_group_button') }}
          </span>
        </b-btn>
        <ConfirmModal :key="'leavegroupmodal-' + data.item.idgroups" :ref="'confirmLeave-' + data.item.idgroups" @confirm="leaveConfirmed(data.item.idgroups)" :message="$translate('groups.leave_group_confirm')" />
      </template>
    </b-table>
  </div>
</template>
