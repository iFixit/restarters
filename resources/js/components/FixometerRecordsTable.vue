<template>
  <div>
    <p class="text-brand small pl-3">{{ $translate('devices.table_intro') }}</p>
    <div class="pl-md-3 pr-md-3">
      ...
      >
        ...
        <template slot="cell(device_category.name)" slot-scope="data">
          {{ $translate('strings.' + data.item.category.name) }}
        </template>
        ...
        <template slot="cell(show_details)" slot-scope="row">
          <div v-if="isAdmin" class="text-md-right">
            ...
            <ConfirmModal :key="'modal-' + row.item.id" ref="confirmDelete" @confirm="deleteConfirmed(row.item)" :message="$translate('devices.confirm_delete')" />
          </div>
  ...
  computed: {
    ...
    fields () {
      let ret = [
        {
          key: 'item_type',
          label: this.$translate('devices.model_or_type'),
          sortable: true,
          tdClass: 'pl-0 pl-md-3'
        },
        {
          key: 'device_category.name',
          label: this.$translate('devices.category'),
      ...
      if (this.powered) {
        ret.push({key: 'brand', label: this.$translate('devices.brand'), sortable: true, thClass: 'd-none d-md-table-cell', tdClass: 'd-none d-md-table-cell'})
      }

      ret.push({key: 'short_problem', label: this.$translate('devices.assessment'), thClass: 'width10 d-none d-md-table-cell', tdClass: 'width10 d-none d-md-table-cell'})
      ret.push({key: 'groupname', label: this.$translate('devices.group'), sortable: true, thClass: 'd-none d-md-table-cell', tdClass: 'd-none d-md-table-cell'})
      ret.push({
        key: 'repair_status',
        label: this.$translate('devices.status'),
        thClass: 'width90px',
        tdClass: 'width90px',
        sortable: true
      })
      ret.push({
        key: 'created_at',
        label: this.$translate('devices.devices_date'),
        thClass: 'width90px',
        tdClass: 'width90px',
        sortable: true
      })
  ...
  methods: {
    ...
    showStatus (data) {
      switch (data.item.repair_status) {
        case FIXED:
          return this.$translate('partials.fixed')
        case REPAIRABLE:
          return this.$translate('partials.repairable')
        case END_OF_LIFE:
          return this.$translate('partials.end')
        default:
          return null
      }
    },
