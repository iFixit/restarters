<template>
  <div class="d-flex flex-wrap event-name">
    <b-form-group class="flex-grow-1 pr-4">
      <label for="event_name" class="moveright">{{ $translate('events.field_event_name') }}:</label>
      <b-input type="text" id="event_name" name="venue" :placeholder="$translate('events.field_event_name_helper')" v-model="currentVenue" :class="{ hasError: hasError, 'pl-3': true }" />
    </b-form-group>
    <div class="align-self-end mb-3 online d-flex">
      <label for="online">{{ $translate('events.online_event_question') }}</label>
