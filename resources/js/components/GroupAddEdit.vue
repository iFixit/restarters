<template>
  <div v-if="ready">
    <h1 v-if="idgroups">{{ $translate('groups.editing') }} <a class="headlink" :href="'/group/view/' + idgroups">{{ name }}</a></h1>
    <h1 v-else>{{ $translate('general.new_group') }}</h1>
    <p v-if="creating">
      {{ $translate('groups.add_groups_content') }}
    </p>
    <p v-else>
      {{ $translate('groups.edit_group_text') }}
    </p>
    ...
    <div class="layout">
      ...
      <div class="form-group group-description">
        <b-form-group>
          <label for="group_desc">{{ $translate('groups.groups_about_group') }}:</label>
          <RichTextEditor
              id="group_desc"
              name="description"
              class="moveright"
      ...
      <b-card v-if="canApprove" no-body class="group-admin">
        ...
        <b-card-body>
          ...
          <div class="mt-2">
            <b-form-group>
              <label for="group_area">{{ $translate('groups.area') }}:</label>
          ...
          <div class="mt-2" v-if="!approved && canApprove">
            <b-form-group>
              <label class="groups-tags-label" for="moderate">
                {{ $translate('groups.approve_group') }}
              </label>
      ...
      <div class="group-buttons text-right">
        <div v-if="failed">
          <p v-if="creating" class="mt-2 text-danger font-weight-bold" v-html="'<div>' + $translate('groups.create_failed') + '</div>'"/>
          <p v-else class="mt-2 text-danger font-weight-bold" v-html="'<div>' + $translate('groups.edit_failed') + '</div>'"/>
        </div>

        <div class="d-flex justify-content-between flex-wrap" v-if="creating">
          <div class="text-right flex-grow-1 mr-4">
            {{ $translate('groups.groups_approval_text') }}
          </div>
          <SpinButton
              icon-name="save"
              :label="$translate('groups.create_group')"
              variant="primary"
              @handle="submit"
          />
        </div>
        <div class="d-flex justify-content-end" v-else>
          <SpinButton
              icon-name="save"
              :label="$translate('groups.edit_group_save_changes')"
              variant="primary"
              @handle="submit"
          />
        </div>
...
export default {
  ...
  computed: {
    ...
    duplicateError () {
      return this.$translate('groups.duplicate', {
        name: this.name
      })
    },
    networkOptions() {
