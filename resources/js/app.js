...
function validateForm() {
	...
	var validation = Array.prototype.filter.call(form, function (form) {
		...
		var valid =
			validCount ===
			jQuery("#step-2").find("input,select").filter("[required]:visible")
				.length;

		if (
			jQuery("#password").length > 0 &&
			jQuery("#password").val().length < 6
		) {
			...
			jQuery("#password").length > 0 &&
			jQuery("#password").val() !== jQuery("#password-confirm").val()
		) {
			jQuery("#password").addClass("is-invalid");
			jQuery("#password-confirm").addClass("is-invalid");
			jQuery(".email-invalid-feedback").show();
...
function formProcess(e) {
	...
	if (jQuery("#step-2.registration__step--active").length > 0) {
		...
		if (targetLabel) {
			targetLabel.scrollIntoView(500);
		}
	}
}
...
function registration() {
	if (
		jQuery("section.registration").length > 0 &&
		jQuery(".alert.alert-danger").length > 0 &&
		jQuery(".is-invalid").length > 0
	) {
		jQuery(".registration__step").removeClass("registration__step--active");
		jQuery(".is-invalid")
			.first()
			.parents(".registration__step")
			.addClass("registration__step--active");
	}
}
...
function loadDropzones() {
	if (jQuery(".dropzoneEl").length > 0) {
		...
		$(".dropzoneEl").each(function (index) {
			...
			if (
				typeof $("#dropzoneEl-" + $(this).data("deviceid"))["0"].dropzone !=
				"undefined"
			) {
				// Do nothing
				// console.log('nothing');
			} else {
				var instanceDropzone = new Dropzone(
					"#dropzoneEl-" + $(this).data("deviceid"),
					{
						...
						init: function () {
							...
							$dropzone
								.find(".dz-message")
								.append(
									"<span>" + field1 + "</span><small>" + field2 + "</small>",
								);
						},
					},
				);
...
jQuery(function () {
	...
	jQuery(".table:not(.table-devices)").find('[data-toggle="popover"]').popover({
		template:
			'<div class="popover popover__table" role="tooltip"><div class="arrow"></div><h3 class="popover-header"></h3><div class="popover-body"></div></div>',
		placement: "top",
	});
...
$("#register-form-submit").on("click", function (e) {
	...
	if (
		$("#consent_gdpr")["0"].checked &&
		$("#consent_future_data")["0"].checked
	) {
...
$("#delete-form-submit").on("click", function (e) {
	...
	if (
		confirm(
			"You are about to delete your account! Are you sure you wish to continue?",
		)
	) {
		$("#delete-form").submit();
	}
...
$(document).ready(function () {
	$(function () {
		$(".btn-calendar-feed").popover({
			...
			delay: { show: 0, hide: 0 },
			template:
				'<div class="popover popover-calendar-feed" role="tooltip"><div class="arrow"></div><div class="popover-body"></div></div>',
			...
			$.ajax({
				...
				success: function (data) {
					...
					new_items.forEach(function (new_item) {
						var label = "";
						if (!new_item.invites) label += "\u{26A0} ";
						label += new_item.name;
	...
	const locale = $("#language-current").html()
...
function copyToClipboard(content_to_copy, element) {
	...
	if (copyTest === true) {
		...
		try {
			...
			var $set_success_message_in_popover = element.attr(
				"data-content",
				message,
			);
		...
		var $set_original_popover_message = element.attr(
			"data-content",
			$original_popover_text,
		);
	} else {
		// Fallback if browser doesn't support .execCommand('copy')
		window.prompt(
...
function tokenFieldCheck() {
	setTimeout(function () {
		var count_tokens = document
			.getElementById("manual_invite_box")
			.value.split(",");
		var disabled = false;
		...
		$("#event-invite-to button, #invite-to-group button").prop(
			"disabled",
			disabled,
		);
	}, 500);
...
$("#manual_invite_box").on("tokenfield:createtoken", function (event) {
	...
	$.each(existingTokens, function (index, token) {
		if (token.value === newval) event.preventDefault();
	});
...
$(document).ready(function () {
	$("textarea#message_to_restarters[name=message_to_restarters]").on(
		"keydown",
		function (event) {
			if (event.which == 13) {
				event.preventDefault();
		...
		if ($(this).val().length > 0) {
			...
			$.ajax({
				...
				success: function (response) {
					$("div.emailtest > .invalid-feedback")
	...
	$("body > .container:not(.container-nav)").css(
		"min-height",
