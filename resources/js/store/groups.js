...
function newToOld(e) {
	...
	let ret = {
		...
		next_event: e.next_event
			? new moment(e.next_event).format("Y-MM-DD")
			: null,
	...
	actions: {
		...
		async getModerationRequired({ commit, rootGetters }, params) {
			...
			let ret = await axios.get(
				"/api/v2/moderate/groups?api_token=" +
					apiToken +
					"&locale=" +
					getLocale(),
			);
		...
		async create({ rootGetters, commit }, params) {
			...
			try {
				...
				let ret = await axios.post(
					"/api/v2/groups?api_token=" + rootGetters["auth/apiToken"],
					formData,
					{
						headers: {
