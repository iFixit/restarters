...
class ProblemTextScrubberTest extends TestCase
{
    ...
    public function test_redacts_phone_numbers_run_onto_a_preceding_word(): void
    {
        // A word-character lookbehind let these through completely unredacted.
        $this->assertEquals('phone-[phone removed]', $this->scrubber->scrub('phone-555-123-4567'));
        $this->assertEquals('mob-[phone removed]', $this->scrubber->scrub('mob-07700-900123'));
    }
    ...
    {
        $result = $this->scrubber->scrub('Call the owner on +1 (555) 123-4567 before scrapping');

        $this->assertEquals('Call the owner on [phone removed] before scrapping', $result);
        $this->assertEquals(1, $this->scrubber->counts()[ProblemTextScrubber::PHONE]);
    }

    public function test_redacts_uk_style_phone_numbers(): void
    {
        $this->assertEquals(
            'Owner [phone removed] collecting',
            $this->scrubber->scrub('Owner 020 7946 0958 collecting')
        );
    }
    ...
    public function test_handles_multiple_redaction_types_in_one_string(): void
    {
        ...
        $this->assertEquals(1, $counts[ProblemTextScrubber::HTML]);
        $this->assertEquals(5, $this->scrubber->totalRedactions());
    }
