...
class Device extends Model implements Auditable
{
    ...
    {
        //Tested
        $tblImages = env('TBL_IMAGES');
        $tblDevices = env('TBL_DEVICES');
        ...
        return DB::select("SELECT * FROM `{$this->table}` AS `d`
                ...
                LEFT JOIN (
                  SELECT * FROM xref
                    ...
                    WHERE object_type = :tblImages AND reference_type = :tblDevices
                  ) AS i ON i.reference = d.iddevices

                WHERE `event` = :event", ['event' => $event, 'tblImages' => $tblImages, 'tblDevices' => $tblDevices]);
    }
    ...
    {
        return $this->hasOne(\App\Models\Category::class, 'idcategories', 'category');
    }

    public function deviceEvent(): HasOne
    {
        return $this->hasOne(\App\Models\Party::class, 'idevents', 'event');
    }

    public function barriers(): BelongsToMany
    {
        return $this->belongsToMany(\App\Models\Barrier::class, 'devices_barriers', 'device_id', 'barrier_id');
    }
